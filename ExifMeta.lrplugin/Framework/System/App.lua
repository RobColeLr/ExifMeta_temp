...
--      <br>    dur (number, default=3) will be clipped into range 1.5-10. If you need greater duration, call multiple times before bezel-time expires.
...
function App:showBezel( opts, fmt, ... )
    ...
    dur = math.min( math.max( dur, 1.5 ), 10 ) -- legal durs are between 1.5 and 10 (I claim numbers less than 1.5 are often too quick to read, and >10 cause Lr bezel error.
...
--- Display info message - default options.
--
--  @usage Info will be displayed in bezel, or pseudo-bezel.
--  @usage nothing returned, never throws an error (hopefully ;-}).
--
...
--
--  @deprecated in favor of App:alertLogE method.
--
...
--
--  @deprecated in favor of App:alertLogW method.
--
--  @param params (table, default={} ) disp/log parameters - see examples.
--
--  @usage Examples:<br>
...
--
function App:parseToolkitId()
    local tkId = self:getInfo( "LrToolkitIdentifier" ) or "nil"
    local leafName = LrPathUtils.extension( tkId )
    local baseName = LrPathUtils.removeExtension( tkId )
    local dot1 = baseName:find( ".", 1, true )
    if dot1 then
        local dot2 = baseName:find( ".", dot1 + 1, true )
        if dot2 then
            local revDom = baseName:sub( 1, dot2 - 1 )
            return tkId, leafName, revDom, baseName
        else
            return tkId, leafName, baseName, baseName
        end
    end
    self:error( "Toolkit ID is '^1', but must begin with reverse domain name, and end with plugin mash, and in between can be whatever else you want.", tkId )
end
