    ...
    repeat
        ...
        elseif spec.dataType == 'function' then
            if str:is( rawValue ) then
                ...
                elseif err then
                    -- app:error( "Function has syntax error: ^1, err ) -- causes plugin not to load ###1.
                    value = nil
                    rawValue = err
                    --Debug.pause( err )
                else
...
function Settings:_initKwTbl( call )
    ...
    initChildren( catalog:getKeywords() ) -- ###1 consider retrofitting new keywords object.
end
--]]
...
function Settings:getViewItems( params )
    ...
    local function closeTab()
        ...
        local tabViewItems = tab:mergeSets( tabViewSpec.viewOptions, viewItems ) -- copying over view-items not most efficient, but need to make copy of view options. ###1
    ...
    local function addViewItem( spec, parentKey ) -- local
        ...
        elseif spec.dataType == 'number' then
            ...
            if spec.viewType == nil then -- or spec.viewType == 'edit_field' then -- so far (@4/Dec/2013 1:49), edit-field has been a default view-type, never specifiable explicitly. ###1
            ...
            elseif spec.viewType == 'smartCollChooser' then
                ...
                cat:getSmartCollectionPopupItems() -- dump items - just init ###1.
                local dataView = vf:static_text( tab:mergeSets( viewData, { -- assure title binding to translate ID to name.
                    ...
                    title = bind {
                        ...
                        transform = function()
                            ...
                            if id and id ~= 0 then
                                local sc, name, path = cat:getSmartCollection( id )
                ...
                local button = vf:push_button {
                    ...
                    action = function( button )
                        app:call( Call:new{ name="Smart Collection Chooser", async=true, main=function( call )
                            ...
                            local items, lookup = cat:getSmartCollectionPopupItems()
            ...
            elseif spec.viewType == 'pubSrvChooser' then
                ...
                cat:getPublishServicePopupItems() -- init ###1
                local dataView = vf:static_text( tab:mergeSets( viewData, { -- assure title binding to translate ID to name.
                    width = dataWidth, -- shared, which unfortunately means expandable.
                    title = bind {
                        bind_to_object = bindTo,
