--[[
        ...
        Base class for implementing custom app interface classes.
        
        Note: This class does not require a delegate for command processing, nor a background thread.
        It is for interfacing to apps that will be slaves to plugin, which will therefore be acting as master to the apps.
        
        Instructions:
        
            - Extend this class to do something useful.
--]]


local RpcApp, dbg = Obect:newClass{ className = 'RpcApp', register = true }



--- Constructor for extending class.
--
function RpcApp:newClass( t )
...
--
function RpcApp:new( t )
    if t and t.delegate then
        -- good
    else
        -- error( "rpc-background requires delegate for command processing." )
    end
    if t.url then
        self.rpc = XmlRpc:new{ url = t.url } -- note: self is class-var, not instance var.
    else
        error( "rpc-background requires url" )
    end
    if t.appPath then -- sub-path
        self.appPath = t.appPath -- name - path is relative to plugin parent.
    else
        error( "rpc-background requires app-path" )
    end
    if t.appName then -- app-name
        self.appName = t.appName
    else
        error( "rpc-background app-name" )
    end
...
function RpcApp:process( call )
    ...
    if sts then
        ...
        if #values then
            -- Debug.lognpp( "first: " .. str:to( values[1] ) )
...
--
function RpcApp:sendAndReceive( command, ... )
    local sts, msgOrVals
...
function RpcApp:finale( call, status, message )
    --[[ Seems the finale is too late... (saved for posterity I guess)
    local sts, msgOrValues = self.rpc:sendAndReceive( 'quit', 0, "Done..." )
    if sts then -- app should not respond if it exited.
        app:logError( "Unable to quit app" )
    end--]]
end
