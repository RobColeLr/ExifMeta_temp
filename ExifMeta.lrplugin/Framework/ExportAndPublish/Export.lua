...
Export.settingsDef = {
    ...
    ['LR_export_destinationPathPrefix'] = { section="Export Location", title="Folder (destination path prefix)", required='depends', dataType='string' }, -- constraint (str:is)? ###1
    ['LR_export_destinationPathSuffix'] = { section="Export Location", title="Subfolder (destination path suffix)", required='depends', dataType='string' }, -- constraint (str:is)? ###1
    ...
    ['LR_tokensArchivedToString2'] = { section="File Naming", title="behind the scenes...", required='no', dataType='any' }, -- ###1 not sure if data-type should be string or table.
    ['LR_exportServiceProviderTitle'] = { section="General", title="Export To", required='no', dataType='string' },
    ['LR_exportServiceProvider'] = { section="General", title="Export To ###1", required='no', dataType='string' },
    ['LR_selectedTextFontFamily' ] = { section="General", title="Font Family ###1", required='no', dataType='string' },
    ['LR_selectedTextFontSize' ] = { section="General", title="Font Size ###1", required='no', dataType='number', constraints={ min=1, max=math.huge } }, -- ###1 constraints
    ['LR_initialSequenceNumber'] = { section="File Naming", title="Start Number", required='no', dataType='number', constraints={ min=1, max=math.huge } }, -- ###1 may be required to satisfy renaming template - if so, you'd best pass it..
    ...
    ['LR_export_postProcessing'] = { section="Post-processing", title="Post-processing", required='no', dataType='string' }, -- depends on export service ###1.
...
-- get constrained setting value, given prospective (current) value, and default that originates in calling context.
--
--  @return value, status
--
        ...
        elseif spec.dataType == 'number' then
            if type( curVal ) == 'number' then
                if tab:is( spec.constraints ) then
                    ...
                    app:error( "Spec has no numeric range constraints for '^1', so dunno if value (^2) is valid.", id, curVal ) -- too severe? ###1
...
function Export:doExport( params )
    ...
    if explicitConstrain ~= "no" then
        ...
        if tab:is( baseline ) then -- note: adding *baseline* settings to passed table (explicit) will make it grow - safer to copy settings.
            for k, v in pairs( explicit ) do -- this assures explicit table existing externally won't grow when baseline settings are added below.
                if settings[k] == nil then
                    settings[k] = explicit[k]
                end
            end
        else
...
function Export:doExportOnCurrentTask( params )
    ...
    local settings = {} -- lotta checking if photo-by-photo exports - consider a re-partitioning ###1
    ...
    setting{ name='LR_tokensArchivedToString2', default=settings.LR_tokens } -- Not sure this does anything, but I don't think it hurts to have it match the "unarchived" tokens. ###1 string?
