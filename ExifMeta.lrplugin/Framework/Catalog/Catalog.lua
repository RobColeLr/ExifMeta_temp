...
--
--  @deprecated         *** use cat update methods instead.
--
...
function Catalog:_update( catFunc, tmo, name, func, ... )
    ...
    repeat -- do all phases.
        local s, other
        -- ###1 trial basis in Lr5, to see if bugs in timeout table are fixed. If no catalog-update bugs during debug by 2014, do re-allow timeout params for Lr5.3+ in non-adv-dbg mode too.
        local lrVerMajor, lrVerMinor = app:lrVersion()
        if lrVerMajor < 5 or lrVerMinor <=2 or not app:isAdvDbgEna() then -- ###3 Lr4 timeout handling is buggy - catastrophically in my case, since phasing progression depends on reliable/consistent error handling,
...
--  @20/Dec/2013 14:03, only used in Stacker.
--
    ...
    for i, photo in ipairs( seedPhotos ) do
        if not photoSet[photo] then
            if lrMeta:getRaw( photo, 'isInStackInFolder', cache ) then -- and photo:getRawMetadata( 'topOfStackInFolderContainingPhoto' ) then - do not require top of stack, since photo may be in opposite orientation in collection.
                local allPhotos = lrMeta:getRaw( photo, 'stackInFolderMembers', cache )
...
--
--  @deprecated - use get-photo-name-disp instead (I like the simpler name, but I recommend migrating to a metadata caching scheme).
--
...
--  @param metadataCache (LrMetadata::Cache, optional) metadata cache (see code below for expected members).
...
function Catalog:assurePluginCollection( name, tries, pluginName )
    local coll = self:assurePluginCollections( { name }, tries, nil, pluginName )
    return coll
end
...
--
--  @deprecated - *** use lr-metadata method instead.
--
...
function Catalog:setCollectionPhotos( coll, photos, tmo )
    ...
    local function setCollPhotos()
        coll:removeAllPhotos() -- these two functions do not need to be in separate phases.
    ...
    local function getSome( from )
        for i, v in ipairs( from ) do
            ...
            if types[typ] then
                ...
                local item = { title=cat:getSourceName( v ), value=v.localIdentifier or "LrCatalog" }
                items[#items + 1] = item
                ...
                Debug.pause( cat:getSourceName( v ), self:getSourceType( v ), typ )
            end
        end
    end
    getSome( roots )
