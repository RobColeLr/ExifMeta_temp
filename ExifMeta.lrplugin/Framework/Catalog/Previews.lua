...
function Previews:_getPreviewInfo( photo, tmo )
    tmo = tmo or 30
...
function Previews:_isPreviewFresh( photo )
    local info, msg = self:_getPreviewInfo( photo )
    if info then
        if not fso:existsAsFile( pdb ) then
            return nil, "File does not exist: "..pdb
        end
...
-- reminder: previews are not freshened in develop module, so switch to library module
-- before bothering with this function.
-- note: this function executes "pseudo-synchronously", such that if command-line/sqlite is hung (it unfortunately does happen sometimes), it
-- will be abandoned (eventually hung command tasks will populate the process list...) - not sure what else to do though. ###3
--
function Previews:isPreviewFresh( photo, tmo )
    local sts, qual
    LrTasks.startAsyncTask( function()
        sts, qual = self:_isPreviewFresh( photo )
    end )
    app:sleep( tmo or 5, .1, function() -- 5 seconds should be more than enough time for sqlite to load, poll preview db, and return a value, but consider passing a larger value if it makes sense (be careful passing smaller values).
...
-- return data and time it took to get it.
-- *** Beware: this function only works in limited circumstances - see in-line comments below.
-- ###2 23/Aug/2013 3:39 - once back online, report bug to Adobe.
...
function Previews:getImage( photo, photoPath, previewFile, prefLevel, minLevel, freshness, icc, profile, meta, orient, mogParam, fmtMeta, ets, assureDir, dontOverwrite, tryNewLr5WayFirst )
    ...
    if imageData then
        ...
        local previewInfo, msg = self:_getPreviewInfo( photo ) -- std tmo = 30 seconds.
        if msg then
            return nil, msg
        end
