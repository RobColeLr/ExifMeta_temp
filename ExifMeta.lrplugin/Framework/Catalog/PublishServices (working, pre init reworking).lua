...
--
function PublishServices:getPublishedPhotoInfo( pluginId )
    if pluginId == nil then
        pluginId = _PLUGIN.id -- for backward compatibility.
    elseif pluginId == 0 then
        pluginId = nil -- for new capability to get published photos for all services.
    ...
    local info = {}
    local pubPhotos = self:getPublishedPhotos( pluginId ) -- ###1 is this ALL photos, including smart collections?
    -- ###1
end
