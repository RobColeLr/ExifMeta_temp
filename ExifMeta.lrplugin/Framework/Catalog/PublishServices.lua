...
--
function PublishServices:init( call )
    app:callingAssert( call, "no call" )
    local cap = call:setCaption( "Initializing publish service info..." )
    -- format of srvInfo:
    ...
    local srvs = catalog:getPublishServices() -- all plugins.
...
--
--  @usage srvInfo format: srv, pluginId, pubColls, pubPhoto
...
--  ###1: I *think* this is only being used by TreeSync Publisher.
...
--  ###1 I don't *think* this is being used anymore (was used in psa I think).
...
--  ###1 @30/Nov/2013 2:05, I can't see this used anywhere anymore.
...
--
--  ###1 this is used for Photooey & TreePub's config-run and maint-run.
...
-- ###1 used by sqliteroom's export-collections.
