...
-- getting obsoleted: ###1 18/Dec/2013 9:54
--
--- Init keyword cache.
--
...
function Keywords:getKeywordPath( keyword, options )
    options = options or { format='slash' }
    local names = {}
    local comps
    if options.format == 'slash' then
    ...
    elseif options.format=='Lr5' then
        return table.concat( names, " < " )
    elseif options.format == 'legacy' then
        return table.concat( names, " > " )
    else
        app:callingError( "Invalid format: ^1", options.format or 'nil' )
    end
end
    ...
    for i, kwNameOrSubpath in ipairs( kwStrings ) do
        ...
        repeat -- messy, but working AFAICT @19/Dec/2013 4:26.
            ...
            if #kwPqNames > 1 or abs then -- multi-comp kw-subpath
                ...
                if rec[kwNameOrSubpath] then
                    assert( #rec[kwNameOrSubpath] > 1, "hmm..." )
