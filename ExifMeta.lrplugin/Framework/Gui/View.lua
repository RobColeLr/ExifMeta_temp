...
--          <br>isEqual ( function( presetId, presetName, presetKey ) - returns s, m; required ) function to determine equality.
        ...
        local function popupChgHdlr( one, two, id, value )
            ...
            app:call( Call:new{ name="Popup Change Handler", async=true, guard=App.guardSilent, main=function( icall )
                ...
                if value == '__save_as__' or value == '__save__' then
                    ...
                    local aName, m = callback.initNewPreset( presetId, presetName, presetKey ) -- sel-names probably doesn't matter, but since it's available..
                    ...
                    if aName then
                        if value == '__save_as__' then
                            ...
                            app:assert( str:is( aName, 'aName' ), "first value returned from callback's init-new-preset function should be a preset name (string)." )
                            local newNames = callback.getPresetNames( presetId, presetNames, aName ) or error( "no names" )
                            --Debug.pause( "setting", presetNamesPref, #newNames, "names for save-as" )
                            app:setPref( presetNamesPref, newNames ) -- save preset names.
                            app:setPref( selectedPresetNamePref, aName ) -- save preset name selected.
                            computePopupItems( newNames, aName ) -- will set popup-sel to match preset-name.
                            props.edited = false
                            app:show{ info="Saved edited '^1'", aName, actionPrefKey = "Saved preset" }
                        end
                    else
...
function View:getLogControls( params )
    ...
    local show = tab:mergeSets( {
        title = "Show Logs",
        action = function()
            app:showLogFile() -- sync or async based on can-yield. ###1 (I think)
        end,
    }, params.showLogsButtonViewOptions )
    local clear = tab:mergeSets( {
        title = "Clear Logs",
        action = function()
            app:clearLogFile() -- sync or async based on can-yield. ###1 (I think)
        end,
